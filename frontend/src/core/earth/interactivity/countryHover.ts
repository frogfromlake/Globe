...
/**
 * Selects a country by its numeric ID.
 * Highlights the country and shows its label and related news.
 */
export async function selectCountryById(
  ...
  for (const cid of selectedCountryIds) {
    selectedFlags[cid - 1] = 0;
  }
  ...
  if (isoCode) {
    ...
    console.warn("Missing ISO code for country:", countryId);
  }
}
